...
out_dir  <- "/groups/umcg-lifelines/tmp02/projects/ov21_0226/lalajaasko/Cog_NonCog/OUTPUT/tables"
...
write_full_ea <- function(wb, ws, left_raw, right_raw, left_sfx, right_sfx, method) {
  row <- 5
  for (sc in scales) {
    ...
    row <- row + 1
    pgi_term     <- paste0("pgi_EA4_", method)
    pgi_sum_term <- paste0("pgi_sum_EA4_", method)
    m1l <- get_vals(left_raw,  prefix, left_sfx[1],  pgi_term)
    m2l <- get_vals(left_raw,  prefix, left_sfx[2],  pgi_term)
    m1r <- get_vals(right_raw, prefix, right_sfx[1], pgi_term)
    m2r <- get_vals(right_raw, prefix, right_sfx[2], pgi_term)
    ...
    row <- row + 1
    p2l <- get_vals(left_raw,  prefix, left_sfx[2],  pgi_sum_term)
    p2r <- get_vals(right_raw, prefix, right_sfx[2], pgi_sum_term)
...
write_decomposed_ea <- function(wb, ws, left_raw, right_raw, left_sfx, right_sfx, method) {
  row <- 5
  for (sc in scales) {
    prefix <- sc[1]; display_name <- sc[2]
    writeData(wb, ws, display_name,                           startCol = 1, startRow = row)
    ...
    for (cog in c("NonCog", "Cog")) {
      ...
      pgi_term <- paste0("pgi_EA3", cog, "_", method)
      m1l <- get_vals(left_raw,  prefix, left_sfx[1],  pgi_term)
      m2l <- get_vals(left_raw,  prefix, left_sfx[2],  pgi_term)
      m1r <- get_vals(right_raw, prefix, right_sfx[1], pgi_term)
      m2r <- get_vals(right_raw, prefix, right_sfx[2], pgi_term)
    ...
    for (cog in c("NonCog", "Cog")) {
      ...
      pgi_sum_term <- paste0("pgi_sum_EA3", cog, "_", method)
      p2l <- get_vals(left_raw,  prefix, left_sfx[2],  pgi_sum_term)
      p2r <- get_vals(right_raw, prefix, right_sfx[2], pgi_sum_term)
      writeData(wb, ws, label,        startCol=1,  startRow=row)
      write_val(wb, ws, p2l$estimate, p2l$p.value, 5, row)
...
write_interaction_full <- function(wb, ws, cbcl_raw, ysr_raw, method) {
  ...
  for (sc in scales) {
    ...
    addStyle(wb, ws, top_sty, rows = row, cols = 1:12, gridExpand = TRUE)
    row <- row + 1
    pgi_term     <- paste0("pgi_EA4_", method, ":female_ctd")
    pgi_sum_term <- paste0("pgi_sum_EA4_", method, ":female_ctd")
    m1c <- get_vals(cbcl_raw, prefix, "1cbcl", pgi_term)
    m2c <- get_vals(cbcl_raw, prefix, "2cbcl", pgi_term)
    m1y <- get_vals(ysr_raw,  prefix, "1ysr",  pgi_term)
    m2y <- get_vals(ysr_raw,  prefix, "2ysr",  pgi_term)
    writeData(wb, ws, "PGI Offspring EA", startCol=1,  startRow=row)
    ...
    row <- row + 1
    p2c <- get_vals(cbcl_raw, prefix, "2cbcl", pgi_sum_term)
    p2y <- get_vals(ysr_raw,  prefix, "2ysr",  pgi_sum_term)
    writeData(wb, ws, "PGI Parental EA", startCol=1,  startRow=row)
...
write_interaction_decomposed <- function(wb, ws, cbcl_raw, ysr_raw, method) {
  writeData(wb, ws, "Sex interaction with", startCol = 1, startRow = 4)
  for (item in list(c(2,"N"),c(3,"Beta"),c(4,"P-value"),c(5,"Beta"),c(6,"P-value"),
                    c(8,"N"),c(9,"Beta"),c(10,"P-value"),c(11,"Beta"),c(12,"P-value"))) {
    writeData(wb, ws, item[2], startCol = as.integer(item[1]), startRow = 4)
  ...
  for (sc in scales) {
    ...
    for (cog in c("NonCog", "Cog")) {
      label <- if (cog == "NonCog") "PGI Offspring EA NonCognitive" else "PGI Offspring EA Cognitive"
      term  <- paste0("pgi_EA3", cog, "_", method, ":female_ctd")
    ...
    for (cog in c("NonCog", "Cog")) {
      label <- if (cog == "NonCog") "PGI Parental EA NonCognitive" else "PGI Parental EA Cognitive"
      term  <- paste0("pgi_sum_EA3", cog, "_", method, ":female_ctd")
...
build_t1 <- function(wb, cbcl_raw, ysr_raw, method) {
  ...
  write_full_ea(wb, "t1.EAfull", cbcl_raw, ysr_raw, c("1cbcl","2cbcl"), c("1ysr","2ysr"), method)
}

build_t2 <- function(wb, cbcl_raw, ysr_raw, method) {
  ...
  write_top_rows(wb, "t2.EAdecomposed", "CBCL", "YSR", "Table 2.",
                 paste("Comparison of the estimated effects of decomposed EA offspring and parental polygenic indices on",
                       "Behavioral scales across CBCL and YSR samples. All regressions include a set of controls.",
                       "P-values are based on family clustered standard errors."))
  write_decomposed_ea(wb, "t2.EAdecomposed", cbcl_raw, ysr_raw, c("1cbcl","2cbcl"), c("1ysr","2ysr"), method)
}

build_t3 <- function(wb, girls_raw, boys_raw, method) {
  ...
  write_full_ea(wb, "t3.cbcl_EAfull_sex", girls_raw, boys_raw,
                c("1girlscbcl","2girlscbcl"), c("1boyscbcl","2boyscbcl"), method)
}

build_t4 <- function(wb, girls_raw, boys_raw, method) {
...
build_t5 <- function(wb, girls_raw, boys_raw, method) {
  ...
  write_full_ea(wb, "t5.ysr_EAfull_sex", girls_raw, boys_raw,
                c("1girlsysr","2girlsysr"), c("1boysysr","2boysysr"), method)
}

build_t6 <- function(wb, girls_raw, boys_raw, method) {
...
build_t7 <- function(wb, cbcl_raw, ysr_raw, method) {
  ...
  addStyle(wb, ws, createStyle(border = "TopBottom", borderStyle = "thin", halign = "center"),
           rows = 3, cols = 2:12, gridExpand = TRUE)
  write_interaction_full(wb, ws, cbcl_raw, ysr_raw, method)
}

build_t8 <- function(wb, cbcl_raw, ysr_raw, method) {
  ...
  addStyle(wb, ws, createStyle(border = "TopBottom", borderStyle = "thin", halign = "center"),
           rows = 3, cols = 2:12, gridExpand = TRUE)
  write_interaction_decomposed(wb, ws, cbcl_raw, ysr_raw, method)
}

build_t9 <- function(wb, wald_ea4_file, wald_ea3_file, method) {
  ...
  for (i in seq_len(nrow(scale_order))) {
    ...
    for (model in c("M1", "M2")) {
      ...
      ea4_d  <- get_wald(df4, outcome, model, paste0("pgi_EA4_", method))
      ea3_nc <- get_wald(df3, outcome, model, paste0("pgi_EA3NonCog_", method))
      ea3_c  <- get_wald(df3, outcome, model, paste0("pgi_EA3Cog_", method))
...
for (method in c("SBayesR")) {
  ...
  cat("Building sheet 1...\n"); build_t1(wb, ea4_cbcl, ea4_ysr, method)
  cat("Building sheet 2...\n"); build_t2(wb, ea3_cbcl, ea3_ysr, method)
  cat("Building sheet 3...\n"); build_t3(wb, girls_cbcl_ea4, boys_cbcl_ea4, method)
  cat("Building sheet 4...\n"); build_t4(wb, girls_cbcl_ea3, boys_cbcl_ea3, method)
  cat("Building sheet 5...\n"); build_t5(wb, girls_ysr_ea4, boys_ysr_ea4, method)
  cat("Building sheet 6...\n"); build_t6(wb, girls_ysr_ea3, boys_ysr_ea3, method)
